  ...
  try {
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        if (!user) { console.error(`User not found: ${customerEmail}`); break; }

        const { data: accountIds } = await supabase.rpc('get_user_account_ids', { _user_id: user.id });
        if (!accountIds || accountIds.length === 0) { console.error(`No account for user: ${user.id}`); break; }
        ...
        console.log(`[STRIPE-WEBHOOK] Subscription created for account ${accountIds[0]}, plan: ${planType}`);
        break;
      }
